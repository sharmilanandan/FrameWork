...
public class MultiThreadDriver extends Thread implements IAutoConst  {
	//create the logger obj every step log folder 
	   String browser;
	   public MultiThreadDriver(String s) {
		browser=s;
	}
	   public void run(){
			WebDriver driver;
...
//			log.info("Enter the url"+url);
//			driver.get(url);
			    MultiThreadDriver d1=new MultiThreadDriver("chrome");
			    d1.start();
			    MultiThreadDriver d2=new MultiThreadDriver("firefox");
			    d2.start();
